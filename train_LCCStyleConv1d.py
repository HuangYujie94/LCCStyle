...
def load_test_style(img_path):
    style_img = tf.read_file(img_path)

    style_img = tf.image.decode_jpeg(style_img, 3)
    shape = tf.shape(style_img)

    style_img = tf.image.resize_images(style_img, [300, 300])
...
class Train(object):
    ...
    def build_model(self):
        ...
        with slim.arg_scope(model.arg_scope()):
            ...
            gen_img, variables = model.LCCStyleConv1d(imgs, style_imgs, reuse=False, name='LCCStyleConv1d')
        ...
        try:
            while not coord.should_stop():
                ...
                if step%10000 == 0:
                    ...
                    if not os.path.exists('gen_img_LCCStyleConv1d'):
                        os.mkdir('gen_img_LCCStyleConv1d')
                    save_img.save_images(gen_img, './gen_img_LCCStyleConv1d/{0}.jpg'.format(step/10000))

                print ('[{}/1600000],loss:{}, content:{},style:{}'.format(step, loss, cl, sl))

                if step % 80000 ==0:
                    if not os.path.exists('model_LCCStyleConv1d'):
                        os.mkdir('model_LCCStyleConv1d')
                    self.save.save(self.sess, './model_LCCStyleConv1d/general{}.ckpt'.format(step/80000))
                if step >= 1600000:
                    break
    ...
    def test(self):
        ...
        test_img_s = load_test_style(test_style_path)
        with slim.arg_scope(model.arg_scope()):

            gen_img, _ = model.LCCStyleConv1d(test_img_c, test_img_s, reuse=False, name='LCCStyleConv1d')

            # load model
            model_path = self.args.transfer_model

            vars = slim.get_variables_to_restore(include=['LCCStyleConv1d'])
